...
function parsePage(value: string | null) {
  ...
  if (n > SIGNATURE_CLASSES_TOTAL_PAGES) return SIGNATURE_CLASSES_TOTAL_PAGES
  return Math.floor(n)
}
...
export function CategorySignatureClassesListing({ lockedSub }: CategorySignatureClassesListingProps) {
  ...
  const handleSubClick = (item: SignatureClassesSubcategory) => {
  ...
  return (
    <div className="category-page">
      <div className="hoot-container">
        <section className="category-page__banner" aria-label="시그니쳐 클래스">
          <img
            className="category-page__banner-img"
            src="/figma/img/category-signature-classes-banner.png"
            alt=""
            aria-hidden="true"
          />
          <h1 className="category-page__banner-title">시그니쳐 클래스</h1>
        </section>
        ...
        {showPager ? (
          <nav className="category-page__pager" aria-label="페이지 탐색">
            ...
            <button
              ...
              className="category-page__pager-arrow"
              aria-label="다음 페이지"
              disabled={page >= SIGNATURE_CLASSES_TOTAL_PAGES}
              onClick={() => goToPage(page + 1)}
            >
