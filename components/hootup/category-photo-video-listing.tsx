...
import {
  PHOTO_VIDEO_TOTAL_PAGES,
  photoVideoDealRoutes,
  photoVideoSubcategories,
  photoVideoSubcategoryRoutes,
  getPhotoVideoCoursesByDeal,
  getPhotoVideoCoursesByPage,
  getPhotoVideoCoursesBySubcategory,
  type PhotoVideoDeal,
  type PhotoVideoSubcategory,
  type CategoryCourse,
} from '@/lib/hootup-category-photo-video'
...
function parsePage(value: string | null) {
  ...
  if (n > PHOTO_VIDEO_TOTAL_PAGES) return PHOTO_VIDEO_TOTAL_PAGES
  return Math.floor(n)
}
...
type CategoryPhotoVideoListingProps = {
  lockedSub?: Exclude<PhotoVideoSubcategory, '전체'>
  lockedDeal?: PhotoVideoDeal
}

export function CategoryPhotoVideoListing({
  lockedSub,
  lockedDeal,
}: CategoryPhotoVideoListingProps) {
  const router = useRouter()
  const pathname = usePathname()
  const searchParams = useSearchParams()
  const page = parsePage(searchParams.get('page'))

  const [sub, setSub] = useState<PhotoVideoSubcategory>(lockedSub ?? '전체')
  ...
  const handleSubClick = (item: PhotoVideoSubcategory) => {
    const route = photoVideoSubcategoryRoutes[item]
  ...
  const courses = useMemo(() => {
    let list: CategoryCourse[] = lockedDeal
      ? getPhotoVideoCoursesByDeal(lockedDeal)
      : lockedSub
        ? getPhotoVideoCoursesBySubcategory(lockedSub)
        : activeSub === '전체'
          ? [...getPhotoVideoCoursesByPage(page)]
          : getPhotoVideoCoursesBySubcategory(activeSub)
  ...
  return (
    <div className="category-page">
      <div className="hoot-container">
        <section className="category-page__banner" aria-label="사진・영상">
          <img
            className="category-page__banner-img"
            src="/figma/img/category-photo-video-banner.png"
            alt=""
            aria-hidden="true"
          />
          <h1 className="category-page__banner-title">사진・영상</h1>
        </section>
        ...
        <section className="category-page__subs" aria-labelledby="subcat-heading">
          ...
          <div className="category-page__subs-list" role="tablist" aria-label="세부 카테고리">
            {photoVideoSubcategories.map((item) => {
              const active =
                item === '전체'
        ...
        <div className="category-page__filters">
          ...
          {showDealFilters ? (
            <>
              <Link
                href={earlybirdOn ? '/category/photo-video' : photoVideoDealRoutes.earlybird}
              ...
              <Link
                href={eventDealOn ? '/category/photo-video' : photoVideoDealRoutes.event}
        ...
        {showPager ? (
          <nav className="category-page__pager" aria-label="페이지 탐색">
            ...
            <button
              ...
              className="category-page__pager-arrow"
              aria-label="다음 페이지"
              disabled={page >= PHOTO_VIDEO_TOTAL_PAGES}
              onClick={() => goToPage(page + 1)}
            >
