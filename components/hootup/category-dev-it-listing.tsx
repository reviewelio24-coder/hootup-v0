...
import {
  ...
  devItDealRoutes,
  devItSubcategories,
  devItSubcategoryRoutes,
  getDevItCoursesByDeal,
  getDevItCoursesByPage,
  getDevItCoursesBySubcategory,
  type DevItDeal,
  type DevItSubcategory,
  type CategoryCourse,
} from '@/lib/hootup-category-dev-it'
...
function parsePage(value: string | null) {
  ...
  if (n > DEV_IT_TOTAL_PAGES) return DEV_IT_TOTAL_PAGES
  return Math.floor(n)
}
...
type CategoryDevItListingProps = {
  lockedSub?: Exclude<DevItSubcategory, '전체'>
  lockedDeal?: DevItDeal
}

export function CategoryDevItListing({
  lockedSub,
  lockedDeal,
}: CategoryDevItListingProps) {
  const router = useRouter()
  const pathname = usePathname()
  const searchParams = useSearchParams()
  const page = parsePage(searchParams.get('page'))

  const [sub, setSub] = useState<DevItSubcategory>(lockedSub ?? '전체')
  ...
  const handleSubClick = (item: DevItSubcategory) => {
    const route = devItSubcategoryRoutes[item]
  ...
  const courses = useMemo(() => {
    let list: CategoryCourse[] = lockedDeal
      ? getDevItCoursesByDeal(lockedDeal)
      : lockedSub
        ? getDevItCoursesBySubcategory(lockedSub)
        : activeSub === '전체'
          ? [...getDevItCoursesByPage(page)]
          : getDevItCoursesBySubcategory(activeSub)
  ...
  return (
    <div className="category-page">
      <div className="hoot-container">
        <section className="category-page__banner" aria-label="개발・IT">
          <img
            className="category-page__banner-img"
            src="/figma/img/category-dev-it-banner.png"
            alt=""
            aria-hidden="true"
          />
          <h1 className="category-page__banner-title">개발・IT</h1>
        </section>
        ...
        <div className="category-page__filters">
          ...
          {showDealFilters ? (
            <>
              <Link
                href={earlybirdOn ? '/category/dev-it' : devItDealRoutes.earlybird}
              ...
              <Link
                href={eventDealOn ? '/category/dev-it' : devItDealRoutes.event}
        ...
        {showPager ? (
          <nav className="category-page__pager" aria-label="페이지 탐색">
            ...
            <button
              ...
              className="category-page__pager-arrow"
              aria-label="다음 페이지"
              disabled={page >= DEV_IT_TOTAL_PAGES}
              onClick={() => goToPage(page + 1)}
            >
