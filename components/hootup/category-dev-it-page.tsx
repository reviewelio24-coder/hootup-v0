...
export function CategoryDevItPage() {
  const router = useRouter()
  const pathname = usePathname()
  const searchParams = useSearchParams()
  const page = parsePage(searchParams.get('page'))

  const [sub, setSub] = useState<DevItSubcategory>('전체')
  ...
  const courses = useMemo(() => {
    let list = [...getDevItCoursesByPage(page)]
    if (sub !== '전체') list = list.filter((c) => c.subcategory === sub)
    ...
    return list
  }, [page, sub, sort, earlybird, eventDeal])
            ...
            {devItSubcategories.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`category-page__chip${active ? ' is-active' : ''}`}
                  onClick={() => setSub(item)}
                >
                  {item}
                </button>
