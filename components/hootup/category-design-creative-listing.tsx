...
import {
  DESIGN_CREATIVE_TOTAL_PAGES,
  designCreativeDealRoutes,
  designCreativeSubcategories,
  designCreativeSubcategoryRoutes,
  getDesignCreativeCoursesByDeal,
  getDesignCreativeCoursesByPage,
  getDesignCreativeCoursesBySubcategory,
  type DesignCreativeDeal,
  type DesignCreativeSubcategory,
  type CategoryCourse,
} from '@/lib/hootup-category-design-creative'
...
function parsePage(value: string | null) {
  ...
  if (n > DESIGN_CREATIVE_TOTAL_PAGES) return DESIGN_CREATIVE_TOTAL_PAGES
  return Math.floor(n)
}
...
type CategoryDesignCreativeListingProps = {
  lockedSub?: Exclude<DesignCreativeSubcategory, '전체'>
  lockedDeal?: DesignCreativeDeal
}

export function CategoryDesignCreativeListing({
  lockedSub,
  lockedDeal,
}: CategoryDesignCreativeListingProps) {
  const router = useRouter()
  const pathname = usePathname()
  const searchParams = useSearchParams()
  const page = parsePage(searchParams.get('page'))

  const [sub, setSub] = useState<DesignCreativeSubcategory>(lockedSub ?? '전체')
  ...
  const handleSubClick = (item: DesignCreativeSubcategory) => {
    const route = designCreativeSubcategoryRoutes[item]
  ...
  const courses = useMemo(() => {
    let list: CategoryCourse[] = lockedDeal
      ? getDesignCreativeCoursesByDeal(lockedDeal)
      : lockedSub
        ? getDesignCreativeCoursesBySubcategory(lockedSub)
        : activeSub === '전체'
          ? [...getDesignCreativeCoursesByPage(page)]
          : getDesignCreativeCoursesBySubcategory(activeSub)
  ...
  return (
    <div className="category-page">
      <div className="hoot-container">
        <section className="category-page__banner" aria-label="디자인・크리에이티브">
          <img
            className="category-page__banner-img"
            src="/figma/img/category-design-creative-banner.png"
            alt=""
            aria-hidden="true"
          />
          <h1 className="category-page__banner-title">디자인・크리에이티브</h1>
        </section>
        ...
        <section className="category-page__subs" aria-labelledby="subcat-heading">
          ...
          <div className="category-page__subs-list" role="tablist" aria-label="세부 카테고리">
            {designCreativeSubcategories.map((item) => {
              const active =
                item === '전체'
        ...
        <div className="category-page__filters">
          ...
          {showDealFilters ? (
            <>
              <Link
                href={earlybirdOn ? '/category/design-creative' : designCreativeDealRoutes.earlybird}
              ...
              <Link
                href={eventDealOn ? '/category/design-creative' : designCreativeDealRoutes.event}
        ...
        {showPager ? (
          <nav className="category-page__pager" aria-label="페이지 탐색">
            ...
            <button
              ...
              className="category-page__pager-arrow"
              aria-label="다음 페이지"
              disabled={page >= DESIGN_CREATIVE_TOTAL_PAGES}
              onClick={() => goToPage(page + 1)}
            >
