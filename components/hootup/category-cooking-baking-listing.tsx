...
import {
  COOKING_BAKING_TOTAL_PAGES,
  cookingBakingDealRoutes,
  cookingBakingSubcategories,
  cookingBakingSubcategoryRoutes,
  getCookingBakingCoursesByDeal,
  getCookingBakingCoursesByPage,
  getCookingBakingCoursesBySubcategory,
  type CookingBakingDeal,
  type CookingBakingSubcategory,
  type CategoryCourse,
} from '@/lib/hootup-category-cooking-baking'
...
function parsePage(value: string | null) {
  ...
  if (n > COOKING_BAKING_TOTAL_PAGES) return COOKING_BAKING_TOTAL_PAGES
  return Math.floor(n)
}
...
type CategoryCookingBakingListingProps = {
  lockedSub?: Exclude<CookingBakingSubcategory, '전체'>
  lockedDeal?: CookingBakingDeal
}

export function CategoryCookingBakingListing({
  lockedSub,
  lockedDeal,
}: CategoryCookingBakingListingProps) {
  const router = useRouter()
  const pathname = usePathname()
  const searchParams = useSearchParams()
  const page = parsePage(searchParams.get('page'))

  const [sub, setSub] = useState<CookingBakingSubcategory>(lockedSub ?? '전체')
  ...
  const handleSubClick = (item: CookingBakingSubcategory) => {
    const route = cookingBakingSubcategoryRoutes[item]
  ...
  const courses = useMemo(() => {
    let list: CategoryCourse[] = lockedDeal
      ? getCookingBakingCoursesByDeal(lockedDeal)
      : lockedSub
        ? getCookingBakingCoursesBySubcategory(lockedSub)
        : activeSub === '전체'
          ? [...getCookingBakingCoursesByPage(page)]
          : getCookingBakingCoursesBySubcategory(activeSub)
  ...
  return (
    <div className="category-page">
      <div className="hoot-container">
        <section className="category-page__banner" aria-label="요리・베이킹">
          <img
            className="category-page__banner-img"
            src="/figma/img/category-cooking-baking-banner.png"
            alt=""
            aria-hidden="true"
          />
          <h1 className="category-page__banner-title">요리・베이킹</h1>
        </section>
        ...
        <section className="category-page__subs" aria-labelledby="subcat-heading">
          ...
          <div className="category-page__subs-list" role="tablist" aria-label="세부 카테고리">
            {cookingBakingSubcategories.map((item) => {
              const active =
                item === '전체'
        ...
        <div className="category-page__filters">
          ...
          {showDealFilters ? (
            <>
              <Link
                href={earlybirdOn ? '/category/cooking-baking' : cookingBakingDealRoutes.earlybird}
              ...
              <Link
                href={eventDealOn ? '/category/cooking-baking' : cookingBakingDealRoutes.event}
        ...
        {showPager ? (
          <nav className="category-page__pager" aria-label="페이지 탐색">
            ...
            <button
              ...
              className="category-page__pager-arrow"
              aria-label="다음 페이지"
              disabled={page >= COOKING_BAKING_TOTAL_PAGES}
              onClick={() => goToPage(page + 1)}
            >
